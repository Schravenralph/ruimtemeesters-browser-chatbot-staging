							...
							>
								<div class="self-center relative">
									<img
										...
										class=" size-[5.25rem] object-cover rounded-full"
...
{#if $showSidebar}
	...
	>
		...
		>
			...
			>
				<a
					class="flex items-center rounded-xl size-[4.55rem] h-full justify-center hover:bg-gray-100/50 dark:hover:bg-gray-850/50 transition no-drag-region"
					href="/"
					draggable="false"
				...
				>
					<img
						...
						src="{WEBUI_BASE_URL}/brand-assets/icon-blue.png"
						class="sidebar-new-chat-icon size-[4.55rem] rounded-full"
						alt="{$WEBUI_NAME} logo"
					/>
				</a>
				...
				<a href="/" class="flex flex-1 px-1.5" on:click={newChatHandler}>
					<div
						...
						class=" self-center text-gray-850 dark:text-white brand-display text-[1.8rem] leading-tight"
					>
						{$WEBUI_NAME}
					</div>
				</a>
			...
			>
				<div class="pb-1.5">
					<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
						<a
							id="sidebar-new-chat-button"
							class="group grow flex items-center space-x-3 rounded-2xl px-2.5 py-2.5 hover:bg-gray-100 dark:hover:bg-gray-900 transition outline-none"
							href="/"
							draggable="false"
							on:click={newChatHandler}
							aria-label={$i18n.t('New Chat')}
						>
							...
							<div class="flex flex-1 self-center translate-y-[0.5px]">
								<div class=" self-center text-lg font-primary">{$i18n.t('New Chat')}</div>
							</div>
					...
					<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
						...
						>
							...
							<div class="flex flex-1 self-center translate-y-[0.5px]">
								<div class=" self-center text-lg font-primary">{$i18n.t('Search')}</div>
							</div>
					...
					{#if ($config?.features?.enable_notes ?? false) && ($user?.role === 'admin' || ($user?.permissions?.features?.notes ?? true))}
						<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
							<a
								...
								class="grow flex items-center space-x-3 rounded-2xl px-2.5 py-2.5 hover:bg-gray-100 dark:hover:bg-gray-900 transition"
								href="/notes"
							...
							>
								<div class="self-center">
									<Note className="size-6" strokeWidth="2" />
								</div>

								<div class="flex self-center translate-y-[0.5px]">
									<div class=" self-center text-lg font-primary">{$i18n.t('Notes')}</div>
								</div>
							</a>
						</div>
					{/if}
					...
					{#if $user?.role === 'admin' || $user?.permissions?.workspace?.models || $user?.permissions?.workspace?.knowledge || $user?.permissions?.workspace?.prompts || $user?.permissions?.workspace?.tools}
						<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
							...
							>
								...
								<div class="flex self-center translate-y-[0.5px]">
									<div class=" self-center text-lg font-primary">{$i18n.t('Workspace')}</div>
								</div>
							</a>
						</div>
					{/if}
					...
					{#if $user !== undefined && $user !== null}
						...
						>
							...
							>
								...
								<div class=" self-center font-medium text-3xl">{$user?.name}</div>
							</div>
						</UserMenu>
					{/if}
				</div>
