...
server.tool(
  ...
  {
    query: z.string().describe("Search query in Dutch, e.g. 'luchtkwaliteit' or 'woningbouw Den Haag'"),
...
server.tool(
  ...
  {
    entity_id: z.string().default('').describe('Optional entity ID for specific entity with neighbors. Leave empty for overview.'),
  },
  async ({ entity_id }) => {
...
server.tool(
  ...
  {
    query: z.string().default('').describe("Search text for rules, e.g. 'bouwhoogte centrum Amsterdam'"),
...
server.tool('get_air_quality', 'Get air quality (luchtkwaliteit) data for a location', {
...
server.tool('get_weather', 'Get current weather data for a location in the Netherlands', {
  location: z.string().default('').describe('Municipality name or location'),
}, async ({ location }) => {
...
server.tool('get_building_data', 'Get 3D building data (3DBAG) including heights and categories', {
...
server.tool('search_documents', 'Search spatial documents and policy maps', {
...
server.tool('search_pdok', 'Search PDOK (Kadaster) national geo-datasets', {
...
server.tool('run_population_forecast', 'Run demographic population forecast using ML ensemble (Prophet, SARIMA, Holt-Winters, State-Space)', {
...
server.tool('get_forecast_results', 'Get cached forecast results for a municipality', {
...
server.tool('run_backtest', 'Run walk-forward backtest to validate forecast accuracy', {
...
server.tool('get_diagnostics', 'Get forecast diagnostics — model performance and data quality', {
  geo_code: z.string().describe("CBS gemeente code"),
}, async ({ geo_code }) => {
...
server.tool('get_model_status', 'Get available forecast models and latest run status', {}, async () => {
...
// Cross-app context
server.tool('context_at_coordinate', 'Get full context at a coordinate — combines Databank documents with Geoportaal spatial rules', {
...
server.tool('context_municipality', 'Get municipality overview — gemeente info, document counts, and rules per project', {
...
// Documents
server.tool('search_documents', 'Full-text search across Databank policy documents', {
...
server.tool('get_document_summary', 'Get summary of a specific document', {
...
// Spatial
server.tool('spatial_rules_at_point', 'Get all DSO spatial rules at a coordinate', {
...
server.tool('solar_potential', 'Get solar energy potential for buildings in an area', {
...
// Knowledge graph
server.tool('search_knowledge_graph', 'Search entities in the knowledge graph by name', {
...
server.tool('get_entity_relations', 'Get a knowledge graph entity with all its relationships', {
...
server.tool('traverse_graph', 'Traverse the knowledge graph from a starting entity', {
  ...
  const text = await apiPost(opts, '/v1/kg/traverse', { startId: start_id, maxDepth: max_depth, direction });
  return { content: [{ type: 'text', text }] };
});
