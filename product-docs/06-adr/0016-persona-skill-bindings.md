...
- **Commercieel adviseur** currently invokes none — consumes scan *outputs* via `Document-Generator` or direct file reads, but does not drive scans. A future commercial-side skill (e.g. `tender-go-no-go`) is a candidate; tracked as out-of-scope here.
