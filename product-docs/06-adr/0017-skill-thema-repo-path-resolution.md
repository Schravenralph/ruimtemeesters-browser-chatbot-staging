...
Skills in `MCP-Servers/packages/memory/skills/company/` resolve a thema slug (e.g. `volkshuisvesting`) to a local thema-repo path by reading **`themas.yaml` in the Scanmethode meta-repo**, located via the **`RM_SCANS_ROOT`** environment variable. The convention is uniform across dev (local Projects folder) and prod (mounted volume). Skills MUST NOT hard-code paths; resolution always goes through the manifest.
...
- Local dev uses `RM_SCANS_ROOT=~/Projects/Ruimtemeesters-Thematische-Beleidsscans/`.
